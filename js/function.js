(function ($) {
    ...
    //     $.ajax({
    //         type: "POST",
    //         url: contactFormUrl, // âœ… now JS gets correct value
    ...
    $.ajax({
        type: "POST",
        url: contactFormUrl, // Passed from Blade
        data: $contactform.serialize(),
        success: function (text) {
            if (text === "success") {
                formSuccess(); // âœ… your success callback
            } else {
                submitMSG(false, text); // âœ… your error handler
            }
        },
        error: function () {
